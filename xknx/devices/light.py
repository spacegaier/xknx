...
class Light(Device):
    ...
    def __str__(self):
        ...
        str_brightness = (
            ...
            else ' brightness="{0}"'.format(self.brightness.group_addr_str())
        )

        str_color = (
            ""
            if not self.supports_color
            else ' color="{0}"'.format(self.color.group_addr_str())
        )

        str_rgbw = (
            ""
            if not self.supports_rgbw
            else ' rgbw="{0}"'.format(self.rgbw.group_addr_str())
        )

        str_tunable_white = (
            ""
            if not self.supports_tunable_white
            else ' tunable white="{0}"'.format(self.tunable_white.group_addr_str())
        )

        str_color_temperature = (
            ""
            if not self.supports_color_temperature
            else ' color temperature="{0}"'.format(
                self.color_temperature.group_addr_str()
            )
        )

        return '<Light name="{0}" ' 'switch="{1}"{2}{3}{4}{5}{6} />'.format(
            self.name,
